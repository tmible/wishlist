...
const createGroupActionHandler = await import(
  '../create-group-action-handler.js',
).then(
  (module) => module.default,
);

describe('groups / bot create group action handler', () => {
  it('should create group', async () => {
    await createGroupActionHandler({ match: [ null, '123' ] });
    verify(createGroup(123));
  });

  it('should update list in chat', async () => {
    const ctx = { match: [ null, null, '456' ] };
    await createGroupActionHandler(ctx);
